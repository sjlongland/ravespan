# Automatically adapted for numpy Jul 31, 2006 by convertcode.py
## Manually adjustments, see VOG
# Further changes by: Bogumil Pilecki
...
Perform Levenberg-Marquardt least-squares minimization, based on MINPACK-1.
 ...
 return list.
   def myfunct(p, fjac=None, x=None, y=None, err=None)
    # Parameter values are passed in "p"
    # If FJAC!=None then partial derivatives must be comptuted.
 ...
 (none are required, keys are case insensitive):
    ...
    'value' - the starting parameter value (but see the START_PARAMS
             parameter for more information,[not available for this 
             environment-VOG 11-08-06] ).
 ...
 constrained to be above 50.
   ...
   Fit the parameters of a model with some noise added. 
   A plot shows the results.
   ...
   preal  = [10, 1.0/3e-2, numpy.pi/6]
   y_true = peval( x, preal )
   y      = y_true + 2 * numpy.randn( len(x) )
   err    = 1.0 + 0.15 * numpy.randn( len(x) )
   
   # Initial estimates for MPFIT
   p0 = [8, 1/2.3e-2, numpy.pi/3]
   fa = {'x':x, 'y':y, 'err':err}
   ...
   pylab.errorbar( x, y, yerr=err, fmt='ro', label="Noisy data" )
   ...
   If you want to to have more control via the PARINFO structure, add
   the following lines after the assignment of 'p0'.
 ...
 Converted to NumPy by Martin Vogelaar, 11-08-2006
"""
 ...
 Outputs:
   ...
   .perror
      ...
      *If* you can assume that the true reduced chi-squared value is unity --
      ...
      #-------------------------- VOG 12-01-07 -------------------
      # In numpy 1.0 these definitions were not compatible anymore
      # Use explicit values for NumPy's dtype
      #-----------------------------------------------------------
      np.Float = np.float64
      np.Int = np.int64
      ...
      # 
      # Gives an error message 'list indices must be integers'
      # if 'parinfo' is empty. Changed by right shaped (npar,2) 
      # array with zero's.
      #----------------------------------------------------------
      if (parinfo is not None):
         limited = self.parinfo(parinfo, 'limited', default=[0,0])
         ...
         limited = self.parinfo(parinfo, 'limited', default=np.zeros((npar,2),np.Int))
         limits = self.parinfo(parinfo, 'limits', default=np.zeros((npar,2),np.Float))
...
#            print self.covar
   ...
   def enorm(self, vec):
        ...
        if (self.fastnorm):
           ...
           # Function dot makes code approx. 6% faster
           #----------------------------------------------------------
           ans = np.sqrt(np.dot(vec,vec))
        else:
           ...
           if mx > agiant or mx < adwarf:
              #-------------------------- VOG 10-08-06-------------------------
              # Orig.   ans = mx * np.sqrt(np.sum((vec/mx)*(vec/mx)))
              #----------------------------------------------------------------
              ans = mx * np.sqrt(np.dot((vec/mx),(vec/mx)))
           else:
              #-------------------------- VOG 10-08-06-------------------------
              # Orig.   ans = np.sqrt(np.sum(vec*vec))
              #----------------------------------------------------------------
              ans = np.sqrt(np.dot(vec,vec))
   ...
   def fdjac2(self, fcn, x, fvec, step=None, ulimited=None, ulimit=None, dside=None,
      ...
      if ifree is None:   ifree = np.arange(len(xall))
      ...
      if step is not None:
         stepi = step[ifree] #np.take(step, ifree)
         wh = np.nonzero(stepi > 0)[0]
         if (len(wh) > 0): np.put(h, wh, np.take(stepi, wh))

      ## if relative step is given, use that
      if (len(dstep) > 0):
         dstepi = dstep[ifree] #np.take(dstep, ifree)
      ...
      #-------------------------- BPi 11-12-08 -------------------------------
      # Statement altered: mask = dside == -1  -->  mask = dside[ifree] == -1
      #-----------------------------------------------------------------------
      mask = dside[ifree] == -1
      ...
      if len(ulimited) > 0 and len(ulimit) > 0:
         ...
         # results in an error because (with numpy) there is
         # an ambiguity in the conditions. The statements
         # 'and' and 'or' are replaced by the array counterparts 
         # | and &
         #------------------------------------------------------------
...

   ...
   def lmpar(self, r, ipvt, diag, qtb, delta, x, sdiag, par=None):
      ...
      if len(wh) > 0:
         #-------------------------- BP 17-12-2008 -------------------
         # orig: nsing = wh
         # change according to IDL mpfit.pro code (http://www.physics.wisc.edu/~craigm/idl/down/mpfit.pro)
         #-------------------------------------------------------------------------------------------------
         nsing = wh[0]
         wa1[wh[0]:] = 0
     #-------------------------- BP 17-12-2008 -------------------
